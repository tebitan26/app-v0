...
export async function GET(req: Request) {
  ...
  // Explicit FK to avoid PGRST201 ambiguity (multiple events relations).
  let query = supabaseAdmin
    .from("ticket_resales")
    .select("id,ticket_id,event_id,price_cents,currency,created_at,events!ticket_resales_event_id_fkey(title,city,start_at)")
    .eq("state", "OPEN")
    .gt("events.start_at", cutoffIso)
  ...
  if (error) {
    console.error("resale_list_fetch_failed", error);
...
export async function POST(req: Request) {
  ...
  const { data: ticket, error: ticketError } = await supabaseAdmin
    .from("tickets")
    .select("id,status,used_at,owner_id,event_id,batch_id,ticket_batches(price_cents,currency)")
    .eq("id", ticketId)
    .single();

  if (ticketError) {
    console.error("resale_list_ticket_lookup_failed", ticketError);
    return NextResponse.json({ error: "internal_error" }, { status: 500 });
  }
  ...
  if (updateError) {
    console.error("resale_list_ticket_update_failed", updateError);
    return NextResponse.json({ error: "internal_error" }, { status: 500 });
  }
  ...
  if (resaleError || !resale) {
    console.error("resale_list_insert_failed", resaleError);
    return NextResponse.json({ error: "internal_error" }, { status: 500 });
  }
  ...
  if (logError) {
    console.error("logs_resale_list_insert_failed", logError);
  }
